...
export class PeriodsService {
    ...
    async getAllPeriods(queryDto: PaginationQueryDto): Promise<{ data: PeriodResponseDto[], total: number }> {
        const { page = 1, limit = 10, order_by = 'created_at', order_direction = 'DESC' } = queryDto;
        this.logger.debug(`Fetching periods - page: ${page}, limit: ${limit}, order by: ${order_by}, direction: ${order_direction}`);
    ...
    async createPeriod(dto: CreatePeriodDto): Promise<PeriodResponseDto> {
        this.logger.debug(`Creating period for year: ${dto.year}, semester: ${dto.semester}, period number: ${dto.period_number}`);

        const existingPeriod = await this.periodRepository.findOne({
            where: { year: dto.year, semester: dto.semester, period_number: dto.period_number },
        });

        if (existingPeriod) {
            this.logger.warn(`Period already exists for year: ${dto.year}, semester: ${dto.semester}, period number: ${dto.period_number}`);
    ...
    async updatePeriod(id: string, dto: UpdatePeriodDto): Promise<PeriodResponseDto> {
        this.logger.debug(`Updating period with ID: ${id}`);

        const period: Period | null = await this.periodRepository.findOne({ where: { id } });
